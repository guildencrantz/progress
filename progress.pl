...
foreach my $repo_name (@repositories) {
  ...
  my $repo = Git::Repository->new( git_dir => "$repo_name/.git" );
  ...
  unless(defined $no_fetch || ! defined $remote_repo) {
    defined $remote_repo ? $repo->run('fetch', $remote_repo) : next;
  }

  $branch = 'master' unless defined $branch;
  my $repo_log_iterator = Git::Repository::Log::Iterator->new($repo, $branch, "--name-status", @git_flags);
...
if ($#combined_log >= 0) {
  ...
  $actual_days = $actual_days == 0 ? 1 : $actual_days;
  ...
  warn "No commits found to visualize.";
}
